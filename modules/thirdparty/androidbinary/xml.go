...
func (f *XMLFile) addNamespacePrefix(ns, name ResStringPoolRef) (string, error) {
	...
	if ns != NilResStringPoolRef {
		...
		if ref != 0 && f.HasString(ref) {
			prefix = f.GetString(ref)
		} else if f.HasString(ns) {
			nsURI := f.GetString(ns)
			if nsURI == "http://schemas.android.com/apk/res/android" {
				prefix = "android"
			} else if nsURI != "" {
				prefix = "ns"
			}
...
func (f *XMLFile) readStartElement(sr *io.SectionReader) error {
	...
	for i := 0; i < int(ext.AttributeCount); i++ {
		...
		var value string
		if attr.RawValue != NilResStringPoolRef && f.HasString(attr.RawValue) {
